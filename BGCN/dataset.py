...
class BasicDataset(Dataset):
    ...
    def load_U_B_interaction(self):
        with open(os.path.join(self.path, self.name, 'user_item_{}.txt'.format(self.task)), 'r') as f:
...
def get_dataset(path, name, task='tune', seed=123):
    ...
    bundle_train_data = BundleTrainDataset(path, name, item_data, assist_data, seed=seed) # user_bundle_{}.txt
    print('finish loading bundle train data')
    bundle_test_data = BundleTestDataset(path, name, bundle_train_data, task=task) # user_bundle_{}.txt
