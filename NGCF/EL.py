...
from utility.test2model import *
import os
...
class NGCF(object):
    def __init__(self, data_config, pretrain_data):
        ...
        *********************************************************
        ...
        # print(self.ua_embeddings.shape)
        # self.u_g_embeddings = tf.add(self.ua_embeddings, np.load('/data/prj2020/zss/ngcf/NGCF/npyData/bgcn_users.npy'))
        # self.pos_i_g_embeddings = tf.add(self.pos_i_g_embeddings, np.load('/data/prj2020/zss/ngcf/NGCF/npyData/bgcn_items.npy'))
    ...
    def _init_weights(self):
        ...
        # all_weights['user_embedding'] = np.concatenate((np.load('/data/prj2020/zss/ngcf/result_steamgame/bgcn_user.npy'),np.load('/data/prj2020/zss/ngcf/result_steamgame/ngcf_user.npy')), axis=1)
        # all_weights['item_embedding'] = np.concatenate((np.load('/data/prj2020/zss/ngcf/result_steamgame/bgcn_item.npy'),np.load('/data/prj2020/zss/ngcf/result_steamgame/ngcf_item.npy')), axis=1)
        all_weights['user_embedding'] = tf.Variable(initial_value=all_weights['user_embedding'], trainable=True,
                                                        name='user_embedding', dtype=tf.float32)
        all_weights['item_embedding'] = tf.Variable(initial_value=all_weights['item_embedding'], trainable=True,
                                                        name='user_embedding', dtype=tf.float32)
        
        # print(all_weights['user_embedding'].shape) #128
        # print(all_weights['item_embedding'].shape) #128
    ...
    def _create_ngcf_embed(self):
        ...
        # print("ego",ego_embeddings)
        all_embeddings = [ego_embeddings]
        # tf.concat([np.load('/data/prj2020/zss/ngcf/NGCF/npyData/bgcn_users.npy'),np.load('/data/prj2020/zss/ngcf/NGCF/npyData/bgcn_items.npy')], 0)
        ...
        for k in range(0, self.n_layers):
            ...
            # print(1, ego_embeddings.shape)
            # message dropout.
            ego_embeddings = tf.nn.dropout(ego_embeddings, 1 - self.mess_dropout[k])
            # print(2, ego_embeddings.shape)
            # normalize the distribution of embeddings.
            norm_embeddings = tf.nn.l2_normalize(ego_embeddings, axis=1)
            # print(3, ego_embeddings.shape)
            all_embeddings += [norm_embeddings]
            # print(all_embeddings)
        # print(all_embeddings)
        # all_embeddings = [tf.concat([all_embeddings[0],all_embeddings[1]],1)+self.bgcn, all_embeddings[2],all_embeddings[3]]
    ...
    def save(self):
        ...
        np.save('./el_user.npy', data_numpy)
        print(data_numpy.shape)
        data_numpy = self.weights['item_embedding'].eval(session=sess)
        np.save('./el_item.npy', data_numpy)
        print(data_numpy.shape)
...
if __name__ == '__main__':
    ...
    *********************************************************
    ...
    if args.save_flag == 1:
        ...
        weights_save_path = '%sELweights/%s/%s/%s/l%s_r%s' % (args.weights_path, args.dataset, model.model_type, layer,
    ...
    *********************************************************
    ...
    if args.pretrain == 1:
        ...
        # pretrain_path = '%sELweights/%s/%s/%s/l%s_r%s' % (args.weights_path, args.dataset, model.model_type, layer,
        #                                                 str(args.lr), '-'.join([str(r) for r in eval(args.regs)]))
        # pretrain_path = './ELweights/newNeatease/ngcf_norm_ngcf_l1/256/l0.0001_r1e-05/'
        pretrain_path = '/data/prj2020/zss/ngcf/result_newNetease/EL_one_256/newNeatease/ngcf_norm_ngcf_l1/256/l0.0001_r1e-05/'
        # pretrain_path = './ELweights/steamgame/ngcf_norm_ngcf_l1/192/l0.0001_r1e-05/'
    ...
    #     t2 = time()
    #     users_to_test = list(data_generator.test_set.keys())
    #     ret = test(sess, model, users_to_test, drop_flag=True) # batch_test
