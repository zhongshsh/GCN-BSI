...
if __name__ == '__main__':
    ...
    if args.save_flag == 1:
        ...
        weights_save_path = '%sNMFweights/%s/%s/%s/l%s_r%s' % (args.proj_path, args.dataset, model.model_type, layer, str(args.lr),
    ...
    if args.pretrain == 1:
        layer = '-'.join([str(l) for l in eval(args.layer_size)])
        pretrain_path = '%sNMFweights/%s/%s/%s/l%s_r%s' % (args.proj_path, args.dataset, model.model_type, layer, str(args.lr),
                                                    '-'.join([str(r) for r in eval(args.regs)]))
