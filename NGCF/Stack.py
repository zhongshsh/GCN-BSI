...
class NGCF(object):
    ...
    def _init_weights(self):
        ...
        # all_weights['user_embedding'] = tf.convert_to_tensor(np.load('/data/prj2020/zss/ngcf/NGCF/npyData/bgcn_items.npy'))
        all_weights['user_embedding'] = tf.Variable(np.load('/data/prj2020/zss/ngcf/result_steamgame/bgcn_user.npy'), trainable=True,
                                                        name='user_embedding', dtype=tf.float32)
        all_weights['item_embedding'] = tf.Variable(np.load('/data/prj2020/zss/ngcf/result_steamgame/bgcn_item.npy'), trainable=True,
                                                        name='item_embedding', dtype=tf.float32)
...
if __name__ == '__main__':
    ...
    *********************************************************
    ...
    if args.save_flag == 1:
        ...
        weights_save_path = '%sStackweights/%s/%s/%s/l%s_r%s' % (args.weights_path, args.dataset, model.model_type, layer,
    ...
    *********************************************************
    ...
    if args.pretrain == 1:
        ...
        pretrain_path = '%sStackweights/%s/%s/%s/l%s_r%s' % (args.weights_path, args.dataset, model.model_type, layer,
                                                        str(args.lr), '-'.join([str(r) for r in eval(args.regs)]))
    ...
    *********************************************************
    ...
    if args.report == 1:
        ...
        report_path = '%sstackreport/%s/%s.result' % (args.proj_path, args.dataset, model.model_type)
