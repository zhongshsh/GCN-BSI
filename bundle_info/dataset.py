...
class BasicDataset(Dataset):
    ...
    def load_U_B_interaction(self):
        with open(
            os.path.join(self.path, self.name, "user_item_{}.txt".format(self.task)),
            "r",
        ) as f:
            return list(
    ...
    def load_U_I_interaction(self):
        with open(os.path.join(self.path, self.name, "user_bundle.txt"), "r") as f:
            return list(
                map(lambda s: tuple(int(i) for i in s[:-1].split("\t")), f.readlines())
            )

    def load_B_I_interaction(self):
        with open(os.path.join(self.path, self.name, "item_bundle.txt"), "r") as f:
            return list(
                map(lambda s: tuple(int(i) for i in s[:-1].split("\t")), f.readlines())
            )
...
class BundleTrainDataset(BasicDataset):
    def __init__(self, path, name, item_data, assist_data, seed=None):
        ...
        if CONFIG["sample"] == "hard":
            ...
            self.u_b_for_neg_sample = np.argsort(u_b_from_i, axis=1)[
                :, bn1_window[0] : bn1_window[1]
            ]

            #  2. b_p --> b_n2
            overlap_graph = (
                assist_data.ground_truth_b_i @ assist_data.ground_truth_b_i.T
            )
            overlap_graph = overlap_graph.todense()
            bn2_window = [int(i * self.num_bundles) for i in CONFIG["hard_window"]]
            self.b_b_for_neg_sample = np.argsort(overlap_graph, axis=1)[
                :, bn2_window[0] : bn2_window[1]
            ]
        ...
        if CONFIG["sample"] == "simple":
            ...
            if hard_probability <= CONFIG["hard_prob"][0]:
                while True:
                    ...
                    if (
                        self.ground_truth_u_b[user_b, b_n1] == 0
                        and not b_n1 in all_bundles
                    ):
                ...
                while True:
                    ...
                    if (
                        self.ground_truth_u_b[user_b, b_n2] == 0
                        and not b_n2 in all_bundles
                    ):
...
def get_dataset(path, name, task="tune", seed=123):
    assist_data = AssistDataset(path, name)  # bundle_item.txt-->
    print("finish loading assist data")
    item_data = ItemDataset(path, name, assist_data, seed=seed)  # user_item.txt
    print("finish loading item data")

    bundle_train_data = BundleTrainDataset(
        path, name, item_data, assist_data, seed=seed
    )  # user_bundle_{}.txt
    print("finish loading bundle train data")
    bundle_test_data = BundleTestDataset(
        path, name, bundle_train_data, task=task
    )  # user_bundle_{}.txt
