...
def main():
    ...
    bundle_test_loader = DataLoader(
        bundle_test_data, 69000, False, num_workers=16, pin_memory=True
    )
    ...
    log = logger.Logger(
        os.path.join(
            CONFIG["log"],
            CONFIG["dataset_name"],
            f"{CONFIG['model']}_{CONFIG['eval_task']}",
            TAG,
        ),
        "best",
    ...
    for DIR in CONFIG["test"]:
        ...
        for i in range(len(d)):
            ...
            if CONFIG["model"] == "bundle_info":
                ...
                info = BGCN_Info(
                    64, dd["embed_L2_norm"], dd["mess_dropout"], dd["node_dropout"], 1
                )
            ...
            model.load_state_dict(
                torch.load(os.path.join(DIR, dd["hash"] + "_Recall@20.pth"))
            )
