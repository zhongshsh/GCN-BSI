...
def graph_generating(raw_graph, row, col):
    if raw_graph.shape == (row, col):
        graph = sp.bmat(
            [
                [sp.identity(raw_graph.shape[0]), raw_graph],
...
class BGCN(Model):
    ...
    def __init__(self, info, dataset, raw_graph, device, pretrain=None):
        # raw_graph =  [ub_graph, ui_graph, bi_graph]
        ...
        # item_size = bi_graph.sum(axis=0) + 1e-8
        # ii_graph @= sp.diags(1/item_size.A.ravel())
        ...
        if ub_graph.shape == (self.num_users, self.num_bundles) and bb_graph.shape == (
            ...
            non_atom_graph = sp.bmat(
                [[sp.identity(ub_graph.shape[0]), ub_graph], [ub_graph.T, bb_graph]]
            )
    ...
    def propagate(self):
        ...
        atom_bundles_feature = F.normalize(
            torch.matmul(self.pooling_graph, atom_items_feature)
        )
    ...
    def forward(self, users, bundles):
        ...
        users_embedding = [
            i[users].expand(-1, bundles.shape[1], -1) for i in users_feature
