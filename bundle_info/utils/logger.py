...
class Logger(object):
    ...
    def __init__(
        ...
        if checkpoint_policy == "always":
            assert checkpoint_interval > 0 and isinstance(checkpoint_interval, int)
    ...
    def update_log(self, metrics, model):
        ...
        with open(
            os.path.join(
                self.root_path, "{}.json".format(self.get_model_Id(self.modelinfo))
            ),
            "w",
        ) as f:
        ...
        if self.checkpoint_policy == "always":
            ...
            for target in self.checkpoint_target:
                if self.metrics_log[target][-1] == max(self.metrics_log[target]):
                    model_path = os.path.join(
                        self.root_path,
                        "{}_{}.pth".format(self.get_model_Id(self.modelinfo), target),
                    )
                    torch.save(model.state_dict(), model_path)
    ...
    def close_log(self, target, window_size=10):
        ...
        best = get_perf(self.metrics_log, window_size=1, target=target, show=False)
        best_str = ", ".join(map(str, best.values()))
        ave = get_perf(
            self.metrics_log, window_size=window_size, target=target, show=False
        )
        ave_str = ", ".join(map(str, ave.values()))
        self.csv_log.write("{}, {}, {}\n".format(best_str, ave_str, str(self.env)))
        self.csv_log.flush()
