...
class NMF(object):
    ...
    def _init_weights(self):
        ...
        for i in range(self.n_layers):
            all_weights["W_%d" % i] = tf.Variable(
                initializer([self.weight_size_list[i], self.weight_size_list[i + 1]]),
                name="W_%d" % i,
            )
        ...
        all_weights["h"] = tf.Variable(
            initializer([self.weight_size_list[-1], 1]), name="h"
        )
    ...
    def _create_inference(self, u_e, i_e):
        ...
        for i in range(self.n_layers):
            ...
            temp = tf.nn.relu(
                tf.matmul(z[i], self.weights["W_%d" % i]) + self.weights["b_%d" % i]
            )
    ...
    def _create_batch_ratings(self, u_e, i_e):
        ...
        if self.model_type == "jrl":
            ...
            z.append(
                tf.reshape(u_1 * i_1, [-1, self.emb_dim])
            )  # (n_b_user * n_b_item, embed_size)
...
def load_pretrained_data():
    pretrain_path = "%sNMFpretrain/%s/%s.npz" % (args.proj_path, args.dataset, "bprmf")
...
if __name__ == "__main__":
    ...
    if args.save_flag == 1:
        ...
        weights_save_path = "%sNMFweights/%s/%s/%s/l%s_r%s" % (
            args.proj_path,
            args.dataset,
            model.model_type,
            layer,
    ...
    if args.pretrain == 1:
        layer = "-".join([str(l) for l in eval(args.layer_size)])
        pretrain_path = "%sNMFweights/%s/%s/%s/l%s_r%s" % (
            args.proj_path,
            args.dataset,
            model.model_type,
            layer,
    ...
    final_perf = (
        "Best Iter=[%d]@[%.1f]\trecall=[%s], precision=[%s], hit=[%s], ndcg=[%s], auc=[%.5f]"
        % (
            idx,
            time() - t0,
    ...
    f.write(
        "embed_size=%d, lr=%.4f, layer_size=%s, keep_prob=%s, regs=%s, loss_type=%s, \n\t%s\n"
        % (
            args.embed_size,
            args.lr,
            args.layer_size,
